...
课程设计教学目的：通过设计并实现一个完整的DNS中继服务器，深入理解DNS协议的工作原理（包括报文格式、域名解析流程等），掌握UDP Socket编程、并发处理、缓存管理、超时重传等核心技术，培养系统设计与调试能力，提升工程规范意识与团队协作能力。

基本目的：实现一个符合RFC 1035标准的DNS中继服务器，支持本地域名映射、不良网站拦截（0.0.0.0）以及向因特网DNS服务器的中继转发；支持多客户端并发查询，正确处理DNS报文ID转换，确保请求与响应的正确匹配；设计并实现DNS缓存，提高解析效率；实现超时重试机制，处理UDP不可靠性带来的丢包问题；提供命令行参数与配置文件两种配置方式，支持多上游服务器轮询负载均衡；代码结构清晰，模块化分层合理，符合工程规范，具备良好的可读性和可维护性。

实验方法：采用C语言，在Linux操作系统下使用GCC编译，基于POSIX Socket API进行网络编程；使用nslookup、ping、自定义并发脚本以及Wireshark抓包进行功能测试和异常报文测试等等。

团队分工：黄严负责整体架构和接口设计、dns层开发、测试；王哲负责基础设施infra层开发、缓存设计、部分函数代码编写、测试；赵梓伊负责server服务器层开发、实验报告撰写、测试。
...
CacheEntry head;           // LRU 哨兵

CacheEntry tail;           // LRU 哨兵
...
/ 若有上游IP参数，注入到server配置段 /

if(ips!=0)

config_inject(SERV_SECTION,KEY_UPSTREAMS,ips);
...
/ 读取超时时间和最大重试次数，首次config_get触发解析 /
...
end->port=SERVER_PORT;  / 统一设置DNS端口 /

h2n_2((uint16_t)&end->port);  / 端口转网络字节序 /
...
result =(T)atol(value);  / 最大重试次数，字符串转长整型 /
...
result =(T)(atol(value)1000);  / 超时秒数转毫秒 /
...
next_timeout = -1;  / 无活跃会话，select无限等待 /

else

/ 计算该会话的剩余超时时间 /

get_session_timeout_remain(earliest_session,request_timeout,&next_timeout);

ex_try();

/ select阻塞等待：socket可读 或 超时 /
...
int ret = pack_recv(&packet,&source_end);
...
ex_throw("pack_recv");  / socket错误 /

return -1;

}

/ 将原始字节流反序列化为DnsPacket结构体 /
...
/ 后置业务检查：A/AAAA记录rdata全0→NXDOMAIN（拦截） /
...
if(g_cache == NULL || qname == NULL || result == NULL)return 1;

/ 构造缓存key："qclass|qtype|qname" /
...
if(mtx_lock(&g_cache->lock)!= thrd_success){ free(key); return 1; }
...
mtx_unlock(&g_cache->lock); free(key); return 1;  / 未命中 /
...
cache_remove_entry(entry);  / 过期则移除 /

mtx_unlock(&g_cache->lock); free(key); return 1;

}

/ 深拷贝CacheValue，调用方可以安全持有和释放 /
...
mtx_unlock(&g_cache->lock); free(key); return 0;
...
if(g_cache == NULL || qname == NULL || cache_value.rrs == NULL)return 1;
...
free_cache_value(&entry->value);  / 释放旧值 /

entry->value = new_value;         / 赋新值 /

entry->created_at = sys_time_ms(); / 更新创建时间 /

lru_touch(g_cache, entry);        / LRU提升 /
...
lru_touch(g_cache, entry);  / 新条目加入LRU头部 /
...
/ 保存客户端原始ID和地址，用于响应时恢复 /

session->client_id = client_id;

session->client_ip = client_ip;

session->relay_info.retry_times = 0;  / 初始重试次数为0 /

session->relay_info.relay_packet = packet_clone(relay_pack);  / 保存中继包副本用于重发 /

/ 以relay_id为key存入HashMap /
...
/ 从Question段提取查询三元组，作为缓存key /

SectionQuestion  q = vector_get(recv->questions,0);

/ 构造CacheValue：三段计数+rrs指针 /
...
value.rrs = recv->rrs;  / 传递原始指针，dns_cache_put内部深拷贝 /
...
/ 重发中继包到上游（轮询选择可能不同的上游） /

if(packet_send(session->relay_info.relay_packet, pick_upstream())==-1){

do_log(ERROR,"timeout resend failed %s",ex_end());

session->relay_info.retry_times--;  / 发送失败不计数 /

}

session->relay_info.retry_times++;  / 重试次数+1 /

session_wait(session);  / 更新时间戳，重新加入优先队列 /
...
1. 在dnsrelay.txt中含有拦截域名（IP设为0.0.0.0）

2. 启动DNS中继服务器并查询被拦截域名：
...
1. 启动DNS中继服务器：./DnsRelay -dd 10.3.9.6
...
1. 使用iptables阻断上游DNS服务器的UDP 53端口通信：

sudo iptables -A OUTPUT -d 8.8.8.8 -p udp --dport 53 -j DROP

启动DNS中继服务器

发起DNS查询，测试完成后恢复iptables：

sudo iptables -D OUTPUT -d 8.8.8.8 -p udp --dport 53 -j DROP

测试结果：

![image16.png](images/image16.png)

查询发出后，服务器创建中继会话（session open），upstream发包后进入等待状态。约1秒后触发首次超时，选择上游服务器重新发送中继包，触发第二次超时，最后关闭会话，超时重试终止。客户端收到系统超时错误。
...
/ 遍历标签，直到遇到压缩指针或域名结尾(0x00)/

while(cur_p[i] &&(cur_p[i] & 0xC0)!= 0xC0)

i += cur_p[i]+1;  / 跳过当前标签（长度字节+内容） /
...
/ 遇到压缩指针：读取2字节，取低14位偏移量 /

uint16_t offset;

memcpy(&offset,(uint16_t )&cur_p[i], 2);

n2h_2(&offset);          / 网络序转主机序 /

offset ^= 0xC000;        / 清除高2位标志位，得到纯偏移量 /

/ 递归解引用：跳转到buf+offset处获取后缀域名 /
...
memcpy(&(res)[i], suffix, strlen(suffix)+ 1);
