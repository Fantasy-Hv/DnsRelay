...
static inline int hash_func_uint64(K key) {
...
typedef K (*KeyExtractor)(T value);

/**
懒删除堆，仅仅是hashmap和最小堆的组合
 *
 */
typedef struct  {
    Comparator comparator;
    HeapElement ** elements; // 带标记的堆数组
    HashMap* survivor_table; //用于O(1)逻辑删除
...
*/
T lazy_heap_pop(LazyHeap* queue);//  如果堆顶元素被标记为删除，就继续pop直到得到有效元素/NULL后返回
